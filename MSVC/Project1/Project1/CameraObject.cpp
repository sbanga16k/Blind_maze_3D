...
void CameraObject::moveCamera()
{
	...
	if (FsGetKeyState(FSKEY_W)) {
		camX_temp = camX + compX * translInc;
		camZ_temp = camZ + compZ * translInc;

		int c = (int) (camZ_temp / scale) ;     // Rows of text file from top to bottom 1-35
		int d = (int) (camX_temp / scale);       // Columns of text file from left to right 1-35
	...
	// Moves in East direction
		...
		camX_temp = camX + compRightX * translInc;
		camZ_temp = camZ + compRightZ * translInc;
